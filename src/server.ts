...
import {buildGitContextSummary, replaceFullContentWithDiffs} from './modules/gitContext';
import {logger as appLogger} from './modules/logger';
import {validateAndLogConfig, applyConfigDefaults} from './modules/configValidator';
import {
  getAllPrompts,
  getPromptOverrides,
  setPromptOverride,
...
export class SimpleProxyServer {
  ...
  constructor() {
    ...
    // Phase B3: Validate config on startup
    const rawConf = configStore.getConfig();
    const validatedConf = validateAndLogConfig(rawConf);
    const conf = applyConfigDefaults(rawConf);
    if (!validatedConf.valid) {
      appLogger.error('[Server] Config validation failed, using defaults', {data: {errors: validatedConf.errors}});
    }
  ...
  private setupMiddleware() {
    // Phase 15 + Task 2: Trace ID middleware — generate correlation ID per request
    this.app.use(async (ctx, next) => {
      const traceId = crypto.randomUUID().slice(0, 12);
      ctx.state.traceId = traceId;
      ctx.set('x-luna-trace-id', traceId);
      await next();
    });

    // Phase B1: Global error handler middleware
    this.app.use(async (ctx, next) => {
      try {
        await next();
      } catch (err) {
        ...
        appLogger.error(`[GlobalError] ${message}`, {
          data: { status, path: ctx.path, method: ctx.method },
  ...
  private setupRoutes() {
    ...
    appLogger.info('[SimpleProxyServer] serving static from', { data: { publicDir } });
    this.app.use(koaStatic(publicDir));

    // Phase A4: Register API routes from extracted routes module
    // This covers: health, config, provider, auth, models, logs, sessions, runs, runtime, network, workers, workspace
    ...
    // Phase 8: Workspace scheduler diagnostics
    this.router.get('/api/workspace/diagnostics', async ctx => {
      ctx.body = getWorkspaceDiagnostics();
    });

    this.router.post('/api/workspace/cleanup-locks', async ctx => {
      const cleaned = cleanupExpiredLocks();
      ctx.body = { ok: true, cleaned };
    });

    // Phase 8: Git context status
    this.router.get('/api/workspace/git-status', async ctx => {
      try {
    ...
    this.router.post('/v1/chat/completions', async ctx => {
      ...
      // Phase 6: Non-stream tool rounds — when messages contain tool results,
      // force non-stream mode to reduce parser issues (90% of parser bugs come from streamed tool calls)
      if (stream) {
        const ccConfig = getClaudeCodeConfig(conf.settings);
        if (shouldUseNonStream(messages, ccConfig)) {
          stream = false;
          appLogger.info('[Server] Claude Code mode: forcing non-stream for tool-call round', { data: { model } });
        }
      }
      const sessionCfg = conf.settings?.session || {};
      ...
      // ---- Phase 8: Session health check & auto-reset ----
      if (currentSession && sessionEnabled) {
        const health = {
          messageCount: currentSession.messages?.length || 0,
          retryCount: 0, // tracked per-session if available
          streamFailureCount: 0,
          lastActivityAt: currentSession.lastRequestAt || currentSession.updatedAt || Date.now(),
          turnCount: currentSession.turnCount || 0,
        };
        const resetDecision = shouldResetSession(health, {
          maxMessages: Number(sessionCfg.maxMessages) || 50,
          staleSessionHours: Number(sessionCfg.staleSessionHours) || 24,
        });
        if (resetDecision.shouldReset) {
          console.log(`[Server] Session ${currentSession.id} auto-reset: ${resetDecision.reason} — ${resetDecision.details}`);
          const resetCtx = createResetContext(combinedMessages);
          // Inject reset context as system message prefix
          combinedMessages = [
            {role: 'system', content: resetCtx.summary},
            ...combinedMessages.slice(-rollingHistoryK),
          ];
          // Clear session's provider binding to force a fresh upstream session
          await sessionStore.resetProviderSessionId(currentSession.id, resetDecision.reason);
          providerSessionId = undefined;
          configStore.addLog('info', JSON.stringify({ path: '/v1/chat/completions', sessionReset: true, reason: resetDecision.reason, details: resetDecision.details, sessionId: currentSession.id }));
        }
      }

      // ---- Phase 8: Context compaction for large sessions ----
      if (combinedMessages.length > (Number(sessionCfg.compactionThreshold) || 30)) {
        const compactionOpts = {
          keepRecentCount: Number(sessionCfg.compactionKeepRecent) || 5,
          maxToolResultChars: Number(sessionCfg.compactionMaxToolChars) || 4000,
          stripThinking: true,
        };
        const compacted = compactMessages(combinedMessages, compactionOpts);
        if (compacted.droppedCount > 0) {
          combinedMessages = buildCompactedMessages(compacted);
          console.log(`[Server] Context compacted: ${compacted.droppedCount} messages dropped, ~${compacted.estimatedTokens} tokens estimated`);
          configStore.addLog('info', JSON.stringify({ path: '/v1/chat/completions', contextCompacted: true, droppedCount: compacted.droppedCount, estimatedTokens: compacted.estimatedTokens, activeTask: compacted.activeTask.slice(0, 100) }));
        }
      }

      // ---- Phase 8: Inject session snapshot into system prompt ----
      if (currentSession && sessionEnabled && currentSession.messages?.length > 0) {
        const snapshot = createSnapshot(combinedMessages);
        if (snapshot.activeTask || snapshot.recentErrors.length > 0 || snapshot.fileReferences.length > 0) {
          combinedMessages = mergeSnapshotWithMessages(combinedMessages, snapshot);
        }
      }

      // ---- Phase 8: Inject git workspace context ----
      try {
        const gitSummary = buildGitContextSummary({ maxDiffChars: 1500, excludePatterns: ['node_modules', '.git', 'dist', 'build'] });
        if (gitSummary) {
          // Prepend git context to the first system message
          const sysIdx = combinedMessages.findIndex(m => m?.role === 'system');
          if (sysIdx >= 0) {
            combinedMessages[sysIdx] = {
              ...combinedMessages[sysIdx],
              content: combinedMessages[sysIdx].content + '\n\n' + gitSummary,
            };
          } else {
            combinedMessages = [{ role: 'system', content: gitSummary }, ...combinedMessages];
          }
        }
      } catch (e) {
        // Git context is non-fatal; silently skip if git is not available
      }
      ...
      let finalized = false;
      // Phase 8: Workspace file lock tracking
      const sessionIdForWorkspace = currentSession?.id || `run-${currentRun.id}`;
      ...
      const finalizeRun = async (status: 'completed' | 'failed' | 'cancelled', extra?: { error?: string; providerChatId?: string; fileBackedSessionId?: string }) => {
        ...
        // Phase 8: Release workspace file locks when run completes
        if (currentSession?.id) {
          releaseSessionLocks(currentSession.id);
        }
        try {
          await releaseRun(currentRun.id);
        } catch (err) {
      ...
      try {
        ...
        if (/429|rate.?limit|too.?many/i.test(errText)) {
          reportRateLimit429(rateAccountKey);
        }
