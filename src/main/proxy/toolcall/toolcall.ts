...
const DEFAULT_TOOL_INSTRUCTIONS = `CRITICAL RULES FOR TOOL USE:

1. IGNORE all built-in tools, hidden tools, native tools, and platform tools.
   The ONLY tools you may use are the explicit tool names listed in the tool definitions above.

2. NEVER output any of these:
   - "Tool does not exist" or "Tool does not exists"
   - "tool resources exhausted" or "tools are unavailable"
   - "I will directly chat instead"
   - role="function" or function_call JSON
   - {"name":...,"arguments":...} format
   - <tool_calls>, <tool_call>, <tool_name>, <parameters> (legacy tags)
   - Partial tags, placeholder names, or commentary about tools
   - \`\`\`xml fences around tool calls
   - Markdown formatting inside tool call XML

3. DO NOT mix reasoning/thinking with tool calls.
   - First: decide which tool to call and with what arguments.
   - Then: output ONLY the complete XML tool call block.
   - Do NOT explain why you are calling the tool.

4. DO NOT output <think> or <thinking> tags when calling a tool.
   If you need to reason, do it silently, then output only the XML.

5. Use ONLY this exact XML structure:
<ml_tool_calls>
  <ml_tool_call>
...
6. Every <ml_tool_call> MUST contain:
   - Exactly one <ml_tool_name> with the EXACT tool name
   - Exactly one <ml_parameters> block with all arguments

7. If you need multiple tools, put all <ml_tool_call> blocks inside ONE <ml_tool_calls>.

8. If you are NOT calling a tool, do not mention XML or tools at all. Answer normally.

9. NEVER emit partial or incomplete XML. Only emit after the entire block is ready.

10. If previous messages contain <ml_tool_result> blocks, use those results to continue.`;

const DEFAULT_TOOL_REMINDER = `[TOOL REMINDER]
Available tools: {{tool_names}}.
When calling a tool: output ONLY complete <ml_tool_calls> XML. No reasoning, no explanation, no markdown fences.
Do NOT output "Tool does not exist" or "tools are unavailable".
Do NOT output <think> tags when calling a tool.
Do NOT mix thinking and tool calls in the same response.`;
