...
export function parseToolCallsStream(
    ...
    if (state.isBufferingToolCall) {
        ...
        // 使用统一解析器处理缓冲内容
        const {content: cleanContent, toolCalls} = parseToolCalls(
            state.contentBuffer,
        );
        ...
        if (toolCalls.length > 0) {
            ...
            if (state.contentBuffer.includes('[/function_calls]')) {
                ...
                state.isBufferingToolCall =
                    state.contentBuffer.includes('[function_calls]');
            }
...
function parseBracketFormat(content: string): {
    ...
    if (
        !processedText.includes('[function_calls]') &&
        missingBracketRegex.test(processedText)
    ) {
    ...
    while ((blockMatch = blockRegex.exec(processedText)) !== null) {
        ...
        while ((callStartMatch = callStartRegex.exec(blockContent)) !== null) {
            ...
            if (parsed) {
                ...
                const rawText = blockContent.substring(
                    callStartMatch.index,
                    rawTextEndIndex,
                );
...
function parseAnthropicFormat(content: string): {
    ...
    const blockRegex =
        /<antml:function_calls>\s*([\s\S]*?)\s*<\/antml:function_calls>/g;
    let blockMatch;
...
export function tryRegexFallback(str: string): any | null {
    try {
        ...
        if (
            ...
            str.includes('"old_str"') &&
            str.includes('"new_str"')
        ) {
            const filePathMatch = str.match(/"filePath"\s*:\s*"([^"]+)"/);
            if (filePathMatch) {
