...
import {attachWatchdog, StreamWatchdog} from '../../../modules/streamWatchdog';
import {sanitizeStreamChunk, sanitizeFullResponse, stripReasoningFromAnswer, deduplicateContent, hasIncompleteToolCalls, autoCloseXmlTags, analyzeResponseQuality} from '../../../modules/responseSanitizer';
import {decideRetry, classifyError, sleep as retrySleep} from '../../../modules/retryPolicy';
import {validateToolCalls, isCompleteToolCall} from '../../../modules/toolCallValidator';
import {writeFileAsync} from '../../../modules/asyncWriter';
import {getRotatingFingerprintHeaders} from '../../../modules/fingerprintRotation';
...
export class QwenAiAdapter {
	...
	private getHeaders(chatId?: string): Record<string, string> {
		...
		// Phase 6: Rotate fingerprint headers to avoid detection
		const fpHeaders = getRotatingFingerprintHeaders();
		Object.assign(headers, fpHeaders);
	...
	private writeWireLog(entry: Record<string, any>): void {
		try {
			...
			// Phase 4: Async write to avoid blocking event loop
			writeFileAsync(filePath, JSON.stringify(entry, null, 2)).catch(err => {
				console.warn('[QwenAI] Failed to async write wire log:', err);
			});
		} catch (err) {
	...
	private tapWireStream(stream: any, meta: Record<string, any>): any {
		try {
			...
			// Phase 6: Use async append instead of sync createWriteStream
			const chunks: string[] = [];
			chunks.push(`# ${JSON.stringify({timestamp: new Date().toISOString(), ...meta, filePath})}\n`);
			...
			stream.on('end', () => {
				chunks.push('\n--- end ---\n');
				writeFileAsync(filePath, chunks.join('')).catch(err => {
					console.warn('[QwenAI] Failed to async write stream log:', err);
				});
			});
			stream.on('close', () => {
				// If stream closed without end, flush what we have
				if (chunks.length > 0) {
					chunks.push('\n--- close ---\n');
					writeFileAsync(filePath, chunks.join('')).catch(() => {});
					chunks.length = 0; // Prevent double write
				}
			});
...
export class QwenAiStreamHandler {
	...
	/** Captured token usage from Qwen stream events */
	...
	private sendToolCalls(transStream: PassThrough): void {
		...
		if (newCalls.length > 0) {
			rawCalls = newCalls.map((tc, i) => ({
				id: tc.id,
				name: tc.name,
				arguments: JSON.stringify(tc.input),
			}));
		} else if (oldCalls.length > 0) {
			rawCalls = oldCalls.map((tc: any) => ({
				id: tc.id || tc.name,
		...
		// Phase 2.5: Validate and repair tool calls before emitting
		const validated = validateToolCalls(rawCalls);
		if (validated.repaired > 0) {
			console.log(`[QwenAI] Tool call validator repaired ${validated.repaired} calls`);
		}
		if (validated.rejected.length > 0) {
			console.warn('[QwenAI] Tool call validator rejected:', validated.rejected.map(r => `${r.name}: ${r.reason}`));
		}
		if (validated.toolCalls.length === 0) {
			console.warn('[QwenAI] No valid tool calls after validation, skipping');
			return;
		}

		const toolCalls = validated.toolCalls.map((tc, i) => ({
			index: i,
			id: tc.id,
		...
		if (!(transStream as any).writableEnded) {
			try {
				const usagePayload = this.lastUsage || {prompt_tokens: 1, completion_tokens: 1, total_tokens: 2};
				transStream.write(
					`data: ${JSON.stringify({
						id: this.responseId || this.chatId,
						model: this.model,
			...
			try {
				transStream.write(
					`data: ${JSON.stringify({
						...
						object: 'chat.completion.chunk',
						choices: [{index: 0, delta: {}, finish_reason: 'tool_calls'}],
						usage: this.lastUsage || {prompt_tokens: 1, completion_tokens: 1, total_tokens: 2},
						created: this.created,
					})}\n\n`,
				);
				transStream.end('data: [DONE]\n\n');
	...
	async handleStream(stream: any): Promise<PassThrough> {
		...
		// Phase 1.4: Attach stream watchdog to detect stalled/dead streams
		let watchdog: StreamWatchdog | null = null;
		let watchdogTriggered = false;
		try {
			watchdog = attachWatchdog(stream, {
				idleTimeoutMs: 25_000,
				label: 'QwenAI-StreamWatchdog',
				onIdle: () => {
					watchdogTriggered = true;
					console.warn('[QwenAI] Watchdog: stream idle for 25s, aborting upstream');
					try {
			...
			console.warn('[QwenAI] Failed to attach stream watchdog:', e);
		}
		...
		// Phase 17: BufferedStreamParser for answer-phase content
		// Replaces raw chunk-level parsing with buffered approach that holds back
		// partial tool call markers until complete blocks are detected
		const bufferedParser = new (require('../../../modules/bufferedToolAccumulator').BufferedStreamParser)();
		...
		const flushPendingAnswerContent = () => {
			...
			// Phase 17: Feed through BufferedStreamParser which holds back partial tool call markers
			// and only emits text once it's confirmed NOT to be a tool call block
			const bufferedResult = bufferedParser.processChunk(content);
			const textToEmit = bufferedResult.textChunks;
			if (textToEmit) {
				// Phase 1.3: Apply stream sanitizer to strip reasoning leaks and leaked content
				let sanitizedContent = sanitizeStreamChunk(textToEmit);
				const visibleContent = this.xmlPassthrough
					? sanitizedContent
					: processStreamChunk(sanitizedContent, toolStreamState).text;
			...
			// If buffered parser detected a complete tool call block, the textChunks is empty
			// but toolCallReady is true — this means we have a complete tool block that
			// will be detected at stream end via the existing this.content accumulation logic
		};
		...
		const parser = createParser({
			onEvent: (event: any) => {
				...
				// Capture token usage from Qwen stream events
				// Qwen may send usage at top-level or inside data
				...
				if (inputTokens > 0 || outputTokens > 0 || totalTokens > 0) {
					...
					console.log('[QwenAI] Captured usage:', JSON.stringify(this.lastUsage));
				}
		...
		stream.once('error', (err: Error) => {
			...
			if (watchdog) { try { watchdog.stop(); } catch {} }
			finalizeStream();
			safeEnd('data: [DONE]\n\n');
		});
		stream.once('close', () => {
		...
		return new Promise((resolve, reject) => {
			...
			const parser = createParser({
				onEvent: (event: any) => {
					...
					// Capture token usage from Qwen stream events (non-stream)
					...
					if (inputTokens > 0 || outputTokens > 0 || totalTokens > 0) {
						...
						console.log('[QwenAI] Captured non-stream usage:', JSON.stringify(this.lastUsage));
						// Update data.usage with real values
						data.usage = this.lastUsage;
					}
					...
					if (parsed.choices && parsed.choices.length > 0) {
							...
							if (phase === 'think' && status !== 'finished') {
								...
								if (status === 'finished') {
									const strippedContent = this.stripProviderToolishContent(data.choices[0].message.content);
									// Phase 1.3: Apply full sanitizer to non-stream answer
									const fullContent = sanitizeFullResponse(strippedContent);
			...
			stream.once('close', () => {
				const rawContent = this.stripProviderToolishContent(data.choices[0].message.content);
				// Phase 1.3: Apply full sanitizer to non-stream response
				const fullContent = sanitizeFullResponse(rawContent);
				data.choices[0].message.content = fullContent;
				if (nativeToolCalls.length > 0) {
	...
	/** Get the last captured real token usage from the stream */
	getLastUsage(): { prompt_tokens: number; completion_tokens: number; total_tokens: number } | null {
