    ...
    <section aria-labelledby="logs-title" className="page-panel logs-panel">
      <div className="page-heading">
        ...
        <div className="action-row">
          ...
          <button className="btn btn-secondary btn-sm" onClick={loadLogs} disabled={loading}>{loading ? t('common.loading') : t('common.refresh')}</button>
          <button className="btn btn-danger btn-sm" onClick={deleteLogs} disabled={deleting}>{deleting ? t('common.deleting') : t('common.delete')}</button>
        </div>
      </div>
                        ...
                        <div>
                          {visible.map((v: any, idx: number) => (
                            <div key={idx} style={{marginBottom: 12}}>
                              <div style={{fontSize: '0.9rem', color: 'var(--text-4)', marginBottom: 6}}>{t('common.message')} {v.index ?? idx}</div>
