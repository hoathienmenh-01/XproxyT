...
export default function Sessions() {
  ...
  return (
    <section aria-labelledby="sessions-title" className="page-panel sessions-panel">
      ...
      {message ? <div className="inline-message info" style={{marginBottom: 'var(--sp-4)'}}>{message}</div> : null}
      {!loading && filtered.length === 0 && diagnostics ? (
        <div className="surface-card" style={{marginBottom: 16, padding: 16}}>
          <h4 style={{marginBottom: 8, color: 'var(--text-1)', fontWeight: 600}}>{t('sessions.emptyTitle')}</h4>
          {!diagnostics.sessionEnabled ? (
            ...
            <div>
              ...
              <details style={{marginTop: 12}}>
                <summary style={{cursor: 'pointer', color: 'var(--text-4)', fontSize: '0.82rem'}}>{t('sessions.diagnostics')}</summary>
          ...
          <tbody>
            {renderedSessions.map(s => (
              ...
              >
                ...
                <td>{s.source}</td>
                <td>{s.workspace || '-'}</td>
                <td style={{fontFamily: 'var(--font-mono)', fontSize: '0.75rem'}}>{s.threadId}</td>
                <td>{s.model || '-'}</td>
                <td>{s.messageCount}</td>
                <td>{s.overflowChain?.length ? s.overflowChain.length : '-'}</td>
                <td style={{fontFamily: 'var(--font-mono)', fontSize: '0.75rem'}}>{s.providerSessionId ? s.providerSessionId.slice(0, 8) + '...' : '-'}</td>
      ...
      {detail && !detailLoading ? (
        <div className="detail-overlay" role="dialog" aria-modal="true" aria-labelledby="session-detail-title">
          ...
          <div style={{marginBottom: 'var(--sp-5)'}}>
            <div className="action-row">
              <button className="btn btn-sm btn-secondary" onClick={() => renameSession(detail.id)}>{t('common.rename')}</button>
              <button className="btn btn-sm btn-secondary" onClick={() => compactSession(detail.id)}>{t('common.compact')}</button>
              <button className="btn btn-sm btn-secondary" onClick={() => clearSession(detail.id)}>{t('common.clear')}</button>
              ...
              <button className="btn btn-sm btn-danger" onClick={() => deleteSession(detail.id)}>{t('common.delete')}</button>
            </div>
          </div>
          <dl className="detail-grid">
            <dt>ID</dt><dd style={{fontFamily: 'var(--font-mono)', fontSize: '0.78rem'}}>{detail.id}</dd>
            ...
            <dt>{t('label.fingerprint')}</dt><dd style={{fontFamily: 'var(--font-mono)', fontSize: '0.78rem'}}>{detail.fingerprint ? detail.fingerprint.slice(0, 16) + '...' : '-'}</dd>
            <dt>{t('label.confidence')}</dt><dd>{detail.confidence || '-'}</dd>
            <dt>{t('label.messages')}</dt><dd>{detail.messageCount}</dd>
            <dt>{t('label.providerChat')} ID</dt><dd style={{fontFamily: 'var(--font-mono)', fontSize: '0.78rem'}}>{detail.providerSessionId || '-'}</dd>
          ...
          {detail.activeRunDetails && detail.activeRunDetails.length > 0 ? (
            <div style={{marginTop: 'var(--sp-5)'}}>
              <h4 style={{color: 'var(--text-1)', fontWeight: 600, fontSize: '0.85rem', marginBottom: 'var(--sp-3)'}}>{t('dashboard.activeRuns')} ({detail.activeRunDetails.length})</h4>
              <div style={{maxHeight: 150, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)'}}>
                {detail.activeRunDetails.map(r => (
                  <div key={r.id} style={{padding: '6px 10px', fontSize: '0.82rem', background: 'var(--bg-raised)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border-2)', display: 'flex', alignItems: 'center', gap: 'var(--sp-3)'}}>
                    <span style={{fontFamily: 'var(--font-mono)', fontSize: '0.75rem'}}>{r.id.slice(0, 8)}</span>
                    <span className="badge badge-accent">{r.status}</span>
                    <span style={{color: 'var(--text-4)', fontSize: '0.78rem'}}>{r.model}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {detail.providerBindings && detail.providerBindings.length > 0 ? (
            <div style={{marginTop: 'var(--sp-5)'}}>
              <h4 style={{color: 'var(--text-1)', fontWeight: 600, fontSize: '0.85rem', marginBottom: 'var(--sp-3)'}}>{t('sessions.providerBindings')} ({detail.providerBindings.length})</h4>
                  ...
                  <tbody>
                    {detail.providerBindings.map((b, i) => (
                      <tr key={i}>
                        <td>{b.providerId}</td>
                        <td>{b.accountId}</td>
                        <td><span className="badge">{b.purpose}</span></td>
                        <td style={{fontFamily: 'var(--font-mono)', fontSize: '0.75rem'}}>{b.providerSessionId ? b.providerSessionId.slice(0, 12) + '...' : '-'}</td>
                        <td>{b.workerId || '-'}</td>
          ...
          {detail.overflowChain && detail.overflowChain.length > 0 ? (
            <div style={{marginTop: 'var(--sp-5)'}}>
              <h4 style={{color: 'var(--text-1)', fontWeight: 600, fontSize: '0.85rem', marginBottom: 'var(--sp-3)'}}>{t('sessions.overflowChain')} ({detail.overflowChain.length})</h4>
              <div style={{maxHeight: 200, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 'var(--sp-1)'}}>
                {detail.overflowChain.map((a, i) => (
                  <div key={i} style={{padding: '4px 8px', fontSize: '0.78rem', background: 'var(--bg-raised)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border-1)'}}>
                    <span style={{color: 'var(--text-5)'}}>[{i}]</span> {a.fileName}
                    <span style={{color: 'var(--text-5)', marginLeft: 8}}>{new Date(a.createdAt).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {detail.summary ? (
            <div style={{marginTop: 'var(--sp-5)'}}>
              <h4 style={{color: 'var(--text-1)', fontWeight: 600, fontSize: '0.85rem', marginBottom: 'var(--sp-3)'}}>{t('sessions.summary')}</h4>
          ...
          {detail.messages && detail.messages.length > 0 ? (
            <div style={{marginTop: 'var(--sp-5)'}}>
              <h4 style={{color: 'var(--text-1)', fontWeight: 600, fontSize: '0.85rem', marginBottom: 'var(--sp-3)'}}>{t('sessions.recentMessages')} ({detail.messages.length})</h4>
              <div style={{maxHeight: 400, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)'}}>
                {detail.messages.slice(-20).map((m, i) => (
                  <div key={m.id || i} style={{padding: '8px 10px', background: 'var(--bg-raised)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border-1)'}}>
                    <div style={{fontSize: '0.75rem', color: 'var(--text-4)', marginBottom: 4, fontFamily: 'var(--font-mono)'}}>
                      [{m.role}] {new Date(m.createdAt).toLocaleTimeString()}
                      {m.runId ? <span style={{marginLeft: 8}}>run: {m.runId.slice(0, 8)}</span> : null}
                      {m.providerId ? <span style={{marginLeft: 8}}>{m.providerId}</span> : null}
                    </div>
                    <pre className="detail-pre" style={{fontSize: '0.78rem', maxHeight: 120, overflow: 'auto'}}>
